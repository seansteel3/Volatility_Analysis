...
* [Methods](#Methods)
  ...
  * [Prerequired Functions](#prerequired-functions)
...
In conclusion, the lack of a clear consistency of which regressions return significant as well as a general lack of real world substantiality to most of the regressions which did return significant, results in a failure to reject the overall null hypothesis that volatility alone, when controlling for daily trade volume, can predict upwards and downwards trends in stock price. However, in some cases it is possible that changes in volatility may yield some predictive power, but as a whole, it would be unwise to make large bets on volatility predicting stock price changes.
